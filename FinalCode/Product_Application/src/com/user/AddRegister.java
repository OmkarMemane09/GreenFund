		...
		try {
			...
			if(role.equalsIgnoreCase("Distribute"))
			{
				...
				((java.sql.Statement) st)
						.executeUpdate("insert into companyregistertbl(Cusername, Cemail, Cpassword, CMbNo,CAddress) values ('"
								+ username
								+ "','"
								+ email
								+ "','"
			...
			Statement st = (Statement) con.createStatement();
			((java.sql.Statement) st)
					.executeUpdate("insert into registertbl(Uusername, Uemail, Upassword, UMbNo,UAddress) values ('"
							+ username
							+ "','"
							+ email
							+ "','"
